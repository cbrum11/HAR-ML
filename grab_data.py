...
for filename in html_dir.iterdir():
    if filename.name[0] != '.': # Ignore hidden mac files
        # Open HTML File
        ...
        for prop_item in prop_item_list:
            ...
            prop_data_dict['list_price'] = (mpi_img.find(class_='block_overlay')
                                            .find(class_='block_price')
                                            .find(class_='price')
                                            .text)

            prop_data_dict['list_status'] = (mpi_img.find(class_='block_overlay')
                                            .find(class_='block_price')
                                            .find(class_='for_status')
                                            .text)
            ...
            for info_type in mpi_info.find(class_='mp_features').find_all(class_='mpf_item'):
                if 'Bed' in info_type.text:
                    ...
                    # Using an unkown_data column to visualize any missed data 
                    # categories from a particular listing
...
# index=False keeps pandas from creating unnecessary
# index column
df.to_csv('output/'+str(currentDT) + '.csv', index=False)